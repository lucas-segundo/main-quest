...
const makeSUT = () => {
  const addSubclassSkillRepo = mockAddSubclassSkillRepository()
  const addSubclassSkillSpy = jest.spyOn(addSubclassSkillRepo, 'add')
  ...
  const sut = new AddSubclassSkillController(
    addSubclassSkillRepo,
    dataValidation,
    httpErrorHandler,
  )
...
describe('AddSubclassSkill', () => {
  it('should call repo with right params', async () => {
    const { sut, addSubclassSkillRepo } = makeSUT()
    const { classID, skillIDs } = mockData()

    await sut.handle(classID, skillIDs)

    expect(addSubclassSkillRepo.add).toHaveBeenCalledWith(classID, skillIDs)
  })

  it('should return 200 and the entity', async () => {
    const { sut, addSubclassSkillSpy } = makeSUT()
    const { classID, skillIDs } = mockData()

    const classWithSkill = mockSubclass()
    addSubclassSkillSpy.mockResolvedValue(classWithSkill)
  ...
  it('should call error handler when error happens', async () => {
    const { sut, addSubclassSkillSpy, httpErrorHandlerSpy } = makeSUT()
    const { classID, skillIDs } = mockData()

    const error = new Error('any_error')
    addSubclassSkillSpy.mockRejectedValue(error)
    await sut.handle(classID, skillIDs)
